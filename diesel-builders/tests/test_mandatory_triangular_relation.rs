...
use diesel::prelude::*;
use diesel_builders::{
    IncompleteBuilderError, TableBuilder, TableBuilderBundle, TrySetColumn, prelude::*,
};
...
diesel::allow_tables_to_appear_in_same_query!(
    parent_table,
    child_with_mandatory_table,
    mandatory_table
);

// Table B models
#[derive(Debug, Queryable, Clone, Selectable, Identifiable, PartialEq, TableModel)]
#[table_model(error=ErrorChildWithMandatory, ancestors = parent_table::table)]
...
impl TrySetColumn<child_with_mandatory_table::remote_mandatory_field>
    for <child_with_mandatory_table::table as TableExt>::NewValues
{
    ...
    fn try_set_column(&mut self, value: Option<String>) -> Result<&mut Self, Self::Error> {
        ...
        self.set_column_unchecked::<child_with_mandatory_table::remote_mandatory_field>(value);
        Ok(self)
    }
}

// Declare singleton foreign key for child_with_mandatory_table::mandatory_id to mandatory_table
fpk!(child_with_mandatory_table::mandatory_id -> mandatory_table);
fpk!(child_with_mandatory_table::id -> parent_table);
...
impl BundlableTable for child_with_mandatory_table::table {
    type MandatoryTriangularColumns = (child_with_mandatory_table::mandatory_id,);
